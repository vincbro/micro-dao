...
def optimize_with_milp(
    ...
    capacity_kw: float,
    initial_storage_kg: float,
    max_storage_kg: float,
    use_solver_heuristics: bool = True,
    ...
    x = pulp.LpVariable.dicts(
        "load_factor", indices, lowBound=0, upBound=1, cat=pulp.LpContinuous
    )
    y = pulp.LpVariable.dicts("is_on", indices, cat=pulp.LpBinary)
    z = pulp.LpVariable.dicts("startup", indices, cat=pulp.LpBinary)
    s = pulp.LpVariable.dicts("storage_level", indices, lowBound=0, upBound=max_storage_kg, cat=pulp.LpContinuous)
    ...
    prob += s[indices[-1]] >= initial_storage_kg, "Terminal_Storage_Requirement"

    # Constraint: hit the hydrogen target exactly
    # prob += (
    #     (
    #         pulp.lpSum(
    #             x[i] * production_rate_kg_per_h * durations_h[i] for i in indices
    #         )
    #         == target_kg
    #     ),
    #     "Production_Target",
    # )
    for i in indices:
        production_i = x[i] * production_rate_kg_per_h * durations_h[i]
        demand_i = snapshots[i].demand_kg
        
        if i == 0:
            prob += s[i] == initial_storage_kg + production_i - demand_i, f"Storage_Balance_{i}"
        else:
            prob += s[i] == s[i-1] + production_i - demand_i, f"Storage_Balance_{i}"
    ...
    for i in range(n):
        for j in range(i + 1, n):
            time_diff_h = (
                interval_starts[j] - interval_starts[i]
    ...
    # Solve
    solver_options = []
    if not use_solver_heuristics:
        solver_options.extend(["-heur", "off"])
    ...
    for snapshot in snapshots:
        ...
        if alloc_sec > 0:
            ...
            if alloc_sec < duration:
                schedule.append(
                    ScheduleState(False, on_end_time, snapshot.end, snapshot)
                )
        else:
            schedule.append(
                ScheduleState(False, snapshot.start, snapshot.end, snapshot)
            )
